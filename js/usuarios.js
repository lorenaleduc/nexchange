...
function atualizarInformacoesUsuario() {
  ...
  const primeiroNome = usuarioAtual.name.split(' ')[0];
  ...
  const administrador = usuarioAtual.role === 'admin';
  ...
  document
    ...
    .classList.toggle('hidden', !administrador);
}
...
function atualizarPerfil(event) {
  ...
  if (administrador) {
    atualizarAdministrador(nome, email, senha);
    return;
  }

  atualizarUsuarioComum(nome, email, senha);
}

function atualizarAdministrador(nome, email, senha) {
  ...
  salvarSessao(usuarioAtual.email);
...
function atualizarUsuarioComum(nome, email, senha) {
  ...
  if (emailJaExiste || email === ADMIN.email) {
  ...
  salvarSessao(usuarioAtualizado.email);
